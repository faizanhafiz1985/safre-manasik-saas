...
const { purgeTenantData: runTenantPurge } = require('../utils/tenantPurge');
const {
  sendEmail,
...
// Purge a tenant's OPERATIONAL/TEST data while KEEPING the tenant, its ADMIN
// users, the built-in system roles, and its settings (system_configs/crm_configs).
// Requires body.confirm === 'PURGE'. Resilient: each step is independent so a
// schema edge case can't abort the whole purge; returns per-table counts.
const purgeTenantData = async (req, res, next) => {
  try {
    const tenantId = req.params.id;
    ...
    const results = await runTenantPurge(prisma, tenantId);
    res.json({ message: `Purged operational data for "${tenant.name}". Tenant, admins, system roles and settings preserved.`, deleted: results });
  } catch (err) {
    next(err);
  }
};
...
module.exports = {
  ...
  suspendTenant, activateTenant, deleteTenant, purgeTenantData, impersonate,
